...
def analyze_cycle(prices):
    ...
    if completion < 35 and last_price >= prev_price:
        phase, comment = "early_accumulation", "Döngü yeni başlamış olabilir."
    elif 35 <= completion < 75 and last_price >= prev_price:
        phase, comment = "trend_expansion", "Döngü orta bölgede."
    elif completion >= 75:
        phase, comment = "late_cycle", "Döngü son bölgeye yaklaşmış."
    else:
        phase, comment = "weak_cycle", "Döngü zayıf ilerliyor."

    return {"phase": phase, "cycle_length": avg_cycle, "cycle_completion": completion, "estimated_days_left": days_left, "last_trough_index": last_trough, "peak_count": len(peaks), "trough_count": len(troughs), "comment": comment}
