async function getAnalysis(symbol: string) {
  const res = await fetch(`http://localhost:8000/api/analyze/${symbol}`, { cache: "no-store" });
  return res.json();
}

async function getScanner() {
  const res = await fetch("http://localhost:8000/api/scanner", { cache: "no-store" });
  return res.json();
}
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1180, margin: "0 auto" }}>
      ...
      <section className="grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
        <div className="card"><h2>{data.symbol}</h2><p style={{ fontSize: 42, margin: 0 }}>{data.bmci_score}</p><span className="badge">{data.signal}</span></div>
        ...
        <div className="grid">
          {scanner.map((item: any) => (
            <div key={item.symbol} style={{ borderBottom: "1px solid #222", padding: "10px 0" }}>
              <strong>{item.symbol}</strong> — BMCI: {item.score} — {item.signal}<br />
